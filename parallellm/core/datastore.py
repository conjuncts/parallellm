...
class DataStore(ABC):
    ...
    def store(self, stage: str, doc_hash: str, response: str, seq_id: Optional[int] = None, save_to_file: bool = True) -> Optional[int]:
        """
...
class ListDataStore(DataStore):
    ...
    def __init__(self, stage_to_resource: dict[str, str]):
        ...
        dataframe should have columns:
            - doc_hash: str
            - response: str
            - seq_id: int (the index in the list)
        """
        self.stage_to_resource = stage_to_resource
            ...
            try:
                ...
                if "seq_id" in df.columns:
                    ...
                    for row in df.iter_rows(named=True):
                        ...
                        while len(self._data[stage]) <= seq_id:
                            self._data[stage].append(None)
                        
                        self._data[stage][seq_id] = response
                        self._hashes[stage][doc_hash] = seq_id
                else:
                    ...
                    for i, row in enumerate(df.iter_rows(named=True)):
                        ...
                        self._data[stage].append(response)
                        self._hashes[stage][doc_hash] = i
                        
            except FileNotFoundError:
                # File doesn't exist yet, initialize empty structures
                self._data[stage] = []
                self._hashes[stage] = {}
    ...
    def store(self, stage: str, doc_hash: str, response: str, seq_id: Optional[int] = None, save_to_file: bool = True) -> int:
        """
        Store a response in the backend.
